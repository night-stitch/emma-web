...
import {
    User, MapPin, Phone, Mail, Wifi, Lock,
        ...
        const fetchData = async () => {
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    setClient(data);
                    setEditData(data); // On pré-remplit le formulaire d'édition
                            ...
                            {missions.length === 0 ? <p className="text-[11px] opacity-40 italic">Aucun historique.</p> :
                                missions.map((m) => (
                                    <div key={m.id} className="p-3 bg-[#F7F5F0] border-l-2 border-[#B88A44] relative group">
                                        ...
                                        <button onClick={() => deleteMission(m.id)} className="absolute top-2 right-2 text-red-400 p-1 md:opacity-0 md:group-hover:opacity-100"><Trash2 size={12} /></button>
                                    </div>
                                ))
                            }
                        </div>
...
export default function ClientDetailsPage() {
    return (
        <Suspense fallback={<div className="p-20 text-center font-serif italic text-[#B88A44]">Accès aux archives...</div>}>
